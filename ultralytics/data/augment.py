...
class Mosaic(BaseMixTransform):
    ...
    def __init__(self, dataset, imgsz=640, p=1.0, n=4, hyp=None):
        ...
        self.hyp = hyp
        if self.hyp is not None and hyp.mask_cutmix > 0:
            self.mask_cutmix = MaskCutMix(hyp.mask_cutmix_crop_dir, 
                                          hyp.mask_cutmix_crop_class_names, 
                                          hyp.mask_cutmix_class_names)
    ...
    def _mosaic4(self, labels):
        ...
        for i in range(4):
            ...
            if self.hyp is not None and random.random() < self.hyp.mask_cutmix:
                img = MaskCutMix.get_tube_img(img)
                
                instances = labels_patch['instances']
                classes = labels_patch['cls']
                
                if classes.shape[0] == 0:
                    classes = np.zeros((0, 1), dtype=classes.dtype)
                    instances = Instances(np.zeros((0, 4), dtype=classes.dtype))
                
                instances.convert_bbox('xywh')
                instances.normalize(img.shape[1], img.shape[0])
                bboxes = instances.bboxes
                labels_cls_xywhn = np.concatenate([classes, bboxes], axis=1)
                
                img, labels_cls_xywhn = self.mask_cutmix(img, labels_cls_xywhn)
                
                new_classes = labels_cls_xywhn[:, 0].reshape(-1, 1)
                new_bboxes = labels_cls_xywhn[:, 1:]
                
                new_instances = Instances(new_bboxes)
                new_instances.denormalize(img.shape[1], img.shape[0])
                new_instances.convert_bbox('xyxy')
                
                labels_patch['instances'] = new_instances
                labels_patch['cls'] = new_classes
...
def v8_transforms(dataset, imgsz, hyp, stretch=False):
    ...
    pre_transform = Compose([
        Mosaic(dataset, imgsz=imgsz, p=hyp.mosaic, hyp=hyp),
...
class MaskCutMix:
    ...
    def __call__(self, im, labels, p=1.0, num_obj=3, **kwargs):
    ...
    def random_paste_img_to_img(self, img: np.ndarray, img_obj: np.ndarray, labels: np.ndarray):
        ...
        for sample_id in range(10):
            ...
            new_bbox = self.warp_bbox(bbox, warp_mat, (height, width))
    ...
    @classmethod
    def get_tube_img(cls, img: np.ndarray) -> np.ndarray:
